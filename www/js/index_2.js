...
function queryDB(tx) {
    tx.executeSql('SELECT * FROM so_giao_banh', [], querySuccess, errorCB);
}
...
function querySuccess(tx, results) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        var so_o = results.rows.item(i).so_o;
        var ngay = results.rows.item(i).ngay;
        var gio = results.rows.item(i).gio;

        trResult += '<tr> <th>'+i+'</th> <td><a href="#" data-rel="external">'+ngay+'</a></td> <td>'+gio+'</td> <td>'+so_o+'</td> </tr>';
    }
    $('#data_sogiaobanh').html = trResult;
}